...
⚡ **ADR numbering contract (GH#8 fix):** create with a PLAIN title `<Title>` — NO `ADR-` prefix, NO `ADR-NNNN:` placeholder. The server auto-assigns the next free number and returns it in the slug (`adr-NNNN-<slug>`). Workflow: (a) `mx_create_doc(title='<Title>', status='active')`; (b) read the number from the returned slug; (c) `mx_update_doc(doc_id, title='ADR-NNNN: <Title>', change_reason='Assign ADR number')` with the REAL number. Never compute the number locally — that would race with parallel creates. ⚡ Historical trap (GH#8): creating with the literal placeholder `ADR-NNNN:` froze the slug as `adr-nnnn-…`, made the doc invisible to the slug-based MAX and caused duplicate numbers. Servers >= build 123 also repair placeholder titles/slugs defensively, but do not rely on it.

**MCP:** `mx_create_doc(project, doc_type='decision', title='<Title>', content, status='active')` — ⚡ `status='active'` is mandatory: the server default is `draft` for every doc_type, and Step 5 below only transitions ADRs whose DB status is `active` (content `proposed`/`accepted` both map to DB `active`, see the mapping table). Plain title, server prepends the ADR number to the slug (see numbering contract above). ⚡ Slug is auto-generated server-side from the title via `GenerateSlug(Title)` at `mx.Tool.Write.pas:541-542`, then the ADR-number prefix is prepended at `mx.Tool.Write.pas:579-585`. The server param `slug=` does not exist on `mx_create_doc` and is silently ignored. Ensure the title is canonical and unique; the server handles dedup via `ClampSlug` + retry-with-suffix (`mx.Tool.Write.pas:588-599`).
