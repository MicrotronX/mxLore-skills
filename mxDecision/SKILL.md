...
⚡ **ADR numbering contract (GH#8 fix):** create with a PLAIN title `<Title>` — NO `ADR-` prefix, NO `ADR-NNNN:` placeholder. The server auto-assigns the next free number and returns it in the slug (`adr-NNNN-<slug>`). Workflow: (a) `mx_create_doc(title='<Title>')`; (b) read the number from the returned slug; (c) `mx_update_doc(doc_id, title='ADR-NNNN: <Title>', change_reason='Assign ADR number')` with the REAL number. Never compute the number locally — that would race with parallel creates. ⚡ Historical trap (GH#8): creating with the literal placeholder `ADR-NNNN:` froze the slug as `adr-nnnn-…`, made the doc invisible to the slug-based MAX and caused duplicate numbers. Servers >= build 123 also repair placeholder titles/slugs defensively, but do not rely on it.
