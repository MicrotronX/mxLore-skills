...
| `agent_watch_session_id` | int OR absent | absent | mxOrchestrate Init 3a (arm) + mxSave Init 4 (clear) / Terminal (re-arm) | Session for which the agent-inbox Monitor is armed. Equal to `session_id` → already armed, do NOT arm a second watcher. Absent or different → arm, then stamp. ⚡ Deliberately a session ID, not a bool: a Monitor dies with its session, so a sticky flag would suppress arming forever after the first session. |
| `agent_watch_task_id` | string OR absent | absent | mxOrchestrate Init 3a (arm) + mxSave Init 4 (clear) / Terminal (re-arm) | Task id of the armed inbox Monitor. ⚡ Required for teardown: `session_id` can rotate inside one process, and a Monitor is a background task that outlives the state edit — without this id the previous watcher cannot be stopped and duplicates accumulate. `TaskStop` it before re-arming; failure is benign. ⚡ **Two writers, one contract:** mxSave takes the watcher down for the duration of a save and puts it back at the end, so both fields are written by mxOrchestrate AND mxSave — always as a PAIR, never one without the other. A task id without a session id cannot be reasoned about; a session id without a task id cannot be torn down. Procedure: `~/.claude/skills/_shared/agent-watch.md`. |
