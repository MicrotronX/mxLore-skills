...
| `agent_watch_session_id` | int OR absent | absent | mxOrchestrate Init step 3a | Session for which the agent-inbox Monitor is armed. Equal to `session_id` → already armed, do NOT arm a second watcher. Absent or different → arm, then stamp. ⚡ Deliberately a session ID, not a bool: a Monitor dies with its session, so a sticky flag would suppress arming forever after the first session. |
| `agent_watch_task_id` | string OR absent | absent | mxOrchestrate Init step 3a | Task id of the armed inbox Monitor. ⚡ Required for teardown: `session_id` can rotate inside one process, and a Monitor is a background task that outlives the state edit — without this id the previous watcher cannot be stopped and duplicates accumulate. `TaskStop` it before re-arming; failure is benign. |
