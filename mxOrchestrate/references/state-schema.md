...
| `last_pruned` | ISO 8601 OR null | `null` | mxSave Step 4b.3 + `orchestrate-reconcile.js` | `null` = never pruned. Stamped whenever the SessionStart hook caps `events_log` (last 10 `synced=true`, all `synced=false` kept) or mxSave Step 4b.3 runs. |
