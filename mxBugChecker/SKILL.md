...
2. Detect VCS → load a **file index, NEVER a whole-tree diff**. Files named as argument→skip **the index only** (the `log`/`status` calls below), go straight to them; the per-file size gate further down still applies to every file you diff, argument or not.
   - ⚡ **Detect by ASKING THE TOOLS, never by looking for a `.git/` or `.svn/` directory.** Since SVN 1.7 `.svn/` exists ONLY in the working-copy root, and `.git/` only in the repo root — a project that sits *inside* a larger checkout has neither, so a directory probe reports "no VCS" for a fully versioned tree. Run both, from wherever you are: `git rev-parse --show-toplevel 2>/dev/null` and `svn info --show-item wc-root 2>/dev/null`. A non-empty answer means that VCS is live here even when its root is several levels above you.
   - ⚡ **Both answer → the DEEPER root wins** (the one closer to your cwd) — that is the repository the project actually belongs to; the outer one just happens to contain it. This is not exotic: a git project checked out inside a large SVN working copy answers both, with the git root several levels below the SVN root. Name the one you used in the report header.
   - ∅argument, git→`git log -5 -- . 2>/dev/null || echo none` + `git status --porcelain -- . 2>/dev/null || echo none` | svn→`svn log -l5 . 2>/dev/null || echo none` + `svn status . 2>/dev/null || echo none`. ⚡ **Scope the index to `.` (the current directory), not to the root** — a root above you may hold many sibling projects, and their changes are not yours to review. Note that the paths printed stay **root-relative** (`src/foo.pas`, not `foo.pas`) even when scoped; that is normal output, not a failed scope — judge the scope by which entries appear, not by how they are spelled. The `|| echo none` is part of the command, not a footnote: an empty/detached repo, or a versioned tree on a machine without the `svn` binary (common on a mounted network share), must degrade to "no index" and continue, never abort the phase.
   - ⚡ **∅VCS is a LIMITATION, not a normal state.** Both probes empty → you cannot see the change set at all: without an argument there is nothing to review, and with one you saw only what you were handed. Say so in the report header — `VCS: none — no change comparison; only the named files were examined` — and never let that run read as a completed clean check. Reporting "no findings" from a blind run is the same silent pass this whole step exists to prevent.
...
```markdown
## /mxBugChecker Report
**Focus:** <Arg or "VCS changes"> | **VCS:** <Git(Branch)|SVN(Rev)|`none — no change comparison; only the named files were examined`>   <!-- never a bare ∅: a blind run must not read like a complete one -->
