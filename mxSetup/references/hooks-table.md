# Hooks Table — mxSetup Phase 5b

Installed into `~/.claude/settings.json` during Phase 5b. Each row lists the event, hooks installed in order, and the runtime they need.

| Event | Hooks (in order) | Language | Requires | Role |
|-------|------------------|----------|----------|------|
| `SessionStart` | `node ~/.claude/hooks/orchestrate-reconcile.js` (2000ms), `node ~/.claude/hooks/orchestrate-status.js` (2000ms) | Node.js | Node.js | Orchestrate reconciles workflow stack vs MCP on session start, then injects status line |
| `UserPromptSubmit` | `node ~/.claude/hooks/orchestrate-status.js` (2000ms) | Node.js | Node.js | Orchestrate status injection on every prompt. Agent messages arrive via the mxMCPProxy session-inbox delivery (proxy >= 1.0.9), not this hook |
| `SubagentStop` | `node ~/.claude/hooks/orchestrate-subagent-flag.js` (2000ms) | Node.js | Node.js | Sets `subagent_ran_since_save: true` in orchestrate-state.json — tracker-gap bridge: subagent MCP-writes bypass the deltas counter; the flag keeps the save-band from reporting a false "nothing unsaved" |
| `Stop` | `node ~/.claude/hooks/orchestrate-step-check.js` (3000ms) | Node.js | Node.js | After-turn step-completion check: mark steps done + sync to MCP |
| `PreToolUse` (matcher: `Edit\|Write`) | `node ~/.claude/hooks/recall-gate.js` (2000ms) | Node.js | Node.js | Recall-Gate: before Edit/Write, check if related lessons/decisions exist and surface them |
| `PreToolUse` (matcher: `Read\|Grep`) | `node ~/.claude/hooks/recall-gate.js --knowledge` (2000ms) | Node.js | Node.js | Knowledge gate: the SAME script as the row above, second branch selected by `--knowledge`. Points at the knowledge dossier for a touched artefact path, so component knowledge is reached by TOUCHING the file rather than by remembering the dossier exists. Watch list = `~/.claude/knowledge-paths.json`, generated by `mxKnowledge`; **absent file = silent no-op**, which is the fresh-install case (the delivered DB is empty, so no dossiers exist yet). ⚡ Cost: ~120 ms per Read/Grep, and Read/Grep is the hottest tool path there is — nearly all of it is the node process start (an empty `node -e ""` measures ~115 ms), so it is the price of ANY node PreToolUse hook, not of this branch. Accepted deliberately: it is the only mechanical trigger; a description-based one does not fire when the model feels no gap |
| `PreToolUse` (matcher: `AskUserQuestion`) | `node ~/.claude/hooks/recall-gate.js --ask` (2000ms) | Node.js | Node.js | Recall-before-asking: the SAME script, third branch. Denies the FIRST user question of a turn when no `mx_search`/`mx_recall`/`mx_detail`/`mx_batch_detail` ran in that turn (read from `transcript_path`); a second attempt in the same turn passes. Reason: documented domain knowledge was once asked for instead of looked up, with a factually wrong answer option offered — a rule alone had not prevented it. Fail-open on an unreadable transcript |
| `PreToolUse` (matcher: `Bash\|PowerShell`) | `node ~/.claude/hooks/env-guard.js` (2000ms) | Node.js | Node.js | .env guard: denies any shell command that could print a VALUE from an `.env`/`*.env`/`.env.*` file (interpreters, `source`, redirects, `git show`, copy-to-stdout …) — the layer `permissions.deny Read(//**/.env…)` (5a-Deny) cannot reach. Allow-listed: `bash ~/.claude/hooks/env-keys.sh <file> [--cmp A B]` (key names + value lengths only) and pure file-management verbs (`ls`, `rm`, `git add/status`, `cp`/`mv` only to an `.env` target or a directory), plus write-only `echo`/`printf` whose `.env` appears solely as `>`/`>>` target (the Read deny also blocks Write/Edit on `.env`, so this is how an `.env` gets created or appended). Fail-closed on globs / `rev:path` / unresolved variables, fail-open on malformed hook input. ⚡ Keep `env-keys.sh` next to it — it is the only sanctioned way to look at an `.env` |
| `PostToolUse` (matcher: `Edit\|Write`) | `node ~/.claude/hooks/recall-outcome-hook.js` (2000ms) | Node.js | Node.js | Recall-Outcome: after Edit/Write, track whether the recalled item was actually useful |

## Node.js degradation (load-bearing)

⚡ Without Node.js, **8 of 11 hooks degrade**:
- Orchestrate reconcile / status / step-check / subagent-flag (4 hooks) → no workflow state tracking, no step auto-complete, no subagent tracker-gap flag
- Recall-Gate + Recall-Outcome (2 hooks) → no pre-edit recall, no outcome tracking
- env-guard (1 hook) → .env values are protected only by `permissions.deny` (Read/Grep/cat…) and the mx-rules text; shell interpreters are no longer intercepted

Only the Bash-based `statusline-command.sh` keeps working.

Installation check (in Phase 5b):
```bash
node --version 2>/dev/null
```
If `node` not found: warn the user, install only the Bash hooks (`statusline-command`), skip the 6 JS hooks, and note that the session runs with limited functionality. Installation pointer: https://nodejs.org/

## Not installed (DORMANT)
`PreCompact` and `PostCompact` prompt-type hooks are **not installed** — see `dormant-precompact.md` for details.
