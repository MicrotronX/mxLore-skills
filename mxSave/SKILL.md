...
**CLAUDE.md:**
- **Weight ⚡ (BYTES, not lines):** check `wc -c CLAUDE.md` AND the longest line (`awk '{print length}' CLAUDE.md | sort -rn | head -1`). Targets: **max 40 KB total** AND **no single line above 4 KB**. Report both numbers, not just a verdict.
