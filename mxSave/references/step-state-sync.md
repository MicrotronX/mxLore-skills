# Step 4a — Step-State Delta Check
...
For each WF in `workflow_stack` where `status='active'` AND `doc_id` is set. Runs REGARDLESS of loop idempotency (this scenario produces no MCP activity — local-only step-flips are EXPECTED to coexist with `total_changes==0`).
...
2. **MCP status guard (race with Step 3):** if `data.status != 'active'` (Step 3 background may have archived this WF in the same tick) → skip, do NOT revive.
...
4. Compare `(local current_step - 1)` vs MCP done-count:
   - `local-1 > MCP done-count` → local ahead, sync needed:
     ...
     - `mx_update_doc(doc_id, content=<full rewritten body>, change_reason='mxSave Step4: step-state rewrite sync', expected_updated_at=data.updated_at)` — `rewrite` keyword bypasses the 50%-length-gate; `expected_updated_at` prevents overwriting a concurrent Step 3 archive.
